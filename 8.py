...
nodes_by_names = [
    ...
    for node, children in (map(lambda x: x.strip(), l.split("=")) for l in lines[1:])
]
...
LEFT_EDGE_TO = [edges[i][0] for i in range(len(idx2node))]

RIGHT_EDGE_TO = [edges[i][1] for i in range(len(idx2node))]
